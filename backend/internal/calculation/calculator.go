...
type BillInput struct {
	...
	TipAmount    int64
	Discount     int64
	DiscountMode domain.DiscountMode
}
...
func Calculate(input BillInput) ([]domain.ParticipantResult, error) {
	...
	if input.ServiceFee < 0 || input.TipAmount < 0 || input.Discount < 0 {
		return nil, errors.New("service fee, tip amount and discount must be non-negative")
	}
	...
	if discountMode != domain.DiscountModeProportional && discountMode != domain.DiscountModeEqual {
		return nil, errors.New("unsupported discount mode")
	}

	participantsByID := make(map[string]domain.Participant, len(input.Participants))
	base := make(map[string]int64, len(input.Participants))
	for _, participant := range input.Participants {
		if participant.ID == "" {
			return nil, errors.New("participant id is required")
		}
		if _, exists := participantsByID[participant.ID]; exists {
			return nil, fmt.Errorf("duplicate participant: %s", participant.ID)
		}
		participantsByID[participant.ID] = participant
		base[participant.ID] = 0
	}

	itemsByID := make(map[string]domain.ReceiptItem, len(input.Items))
	for _, item := range input.Items {
		if item.ID == "" {
			return nil, errors.New("item id is required")
		}
		if item.Total <= 0 {
			return nil, fmt.Errorf("item total must be positive: %s", item.Name)
		}
		if _, exists := itemsByID[item.ID]; exists {
			return nil, fmt.Errorf("duplicate item: %s", item.ID)
		}
		itemsByID[item.ID] = item
	}

	assignmentsByItem := make(map[string][]domain.ItemAssignment)
	seenAssignments := make(map[string]struct{}, len(input.Assignments))
	for _, assignment := range input.Assignments {
		...
		if _, exists := itemsByID[assignment.ItemID]; !exists {
			return nil, fmt.Errorf("assignment references unknown item: %s", assignment.ItemID)
		}
		if _, exists := participantsByID[assignment.ParticipantID]; !exists {
			return nil, fmt.Errorf("assignment references unknown participant: %s", assignment.ParticipantID)
		}

		key := assignment.ItemID + "\x00" + assignment.ParticipantID
		...
		assignmentsByItem[assignment.ItemID] = append(assignmentsByItem[assignment.ItemID], assignment)
	}
	...
	maximumDiscount := subtotal + input.ServiceFee + input.TipAmount
	if input.Discount > maximumDiscount {
		return nil, fmt.Errorf("discount exceeds bill total: maximum is %d", maximumDiscount)
	}

	serviceShares := splitProportionally(input.ServiceFee, base)
	tipShares := splitProportionally(input.TipAmount, base)

	gross := make(map[string]int64, len(base))
	for participantID, amount := range base {
		gross[participantID] = amount + serviceShares[participantID] + tipShares[participantID]
	}

	var discountShares map[string]int64
	switch discountMode {
	case domain.DiscountModeEqual:
		discountShares = splitEquallyWithCapacity(input.Discount, gross)
	default:
	...
	for _, participant := range input.Participants {
		...
		result.TotalAmount = result.BaseAmount + result.ServiceShare + result.TipShare - result.DiscountShare
		if result.TotalAmount < 0 {
			return nil, fmt.Errorf("negative participant total for %s", participant.Name)
		}
...
func BuildDebts(
	...
	var payer *domain.ParticipantResult
	for i := range results {
		if results[i].ParticipantID == payerParticipantID {
			payer = &results[i]
			break
		}
	}
	if payer == nil {
		return nil, errors.New("payer is not a participant")
	}

	debts := make([]domain.Debt, 0, len(results)-1)
	for _, result := range results {
		if result.ParticipantID == payerParticipantID || result.TotalAmount <= 0 {
			continue
		}
	...
	sort.Slice(debts, func(i, j int) bool {
		if debts[i].Amount == debts[j].Amount {
			return debts[i].FromParticipantID < debts[j].FromParticipantID
		}
...
func splitProportionally(total int64, base map[string]int64) map[string]int64 {
	...
	remainders := make([]remainder, 0, len(base))
	var distributed int64
	for participantID, amount := range base {
...
func splitEquallyWithCapacity(total int64, capacity map[string]int64) map[string]int64 {
	...
	remainingCapacity := make(map[string]int64, len(capacity))
	active := make([]string, 0, len(capacity))
	for participantID, amount := range capacity {
		if amount > 0 {
			remainingCapacity[participantID] = amount
	...
	for remaining > 0 && len(active) > 0 {
		...
		extra := remaining % int64(len(active))
		if share == 0 {
			share = 1
			extra = 0
		}

		distributedThisRound := int64(0)
		nextActive := make([]string, 0, len(active))
		for index, participantID := range active {
			desired := share
			if share > 0 && int64(index) < extra {
				desired++
			}
			if desired > remaining-distributedThisRound {
				desired = remaining - distributedThisRound
			}
			if desired <= 0 {
				if remainingCapacity[participantID] > 0 {
					nextActive = append(nextActive, participantID)
				}
				continue
			}

			actual := desired
			if actual > remainingCapacity[participantID] {
				actual = remainingCapacity[participantID]
			}

			result[participantID] += actual
			remainingCapacity[participantID] -= actual
...
func zeroShares(source map[string]int64) map[string]int64 {
	result := make(map[string]int64, len(source))
	for participantID := range source {
